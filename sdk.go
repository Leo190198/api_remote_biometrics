...
// novaInputHandleNativa monta um NBioAPI_INPUT_FIR onde a uniao guarda o
// proprio handle, em vez de um ponteiro.
//
// E a estrutura mais simples que o SDK aceita: dois campos de 4 bytes, nenhum
// ponteiro para seguir. Serve para responder se o gancho de API do servidor RDP
// quebra qualquer VerifyMatch ou so a forma de texto, que envolve ponteiro para
// ponteiro para string e e bem mais dificil de marshalar.
//
// A uniao guarda NBioAPI_FIR_HANDLE*, o endereco de onde o handle esta - nao o
// handle. Foi o proprio SDK que disse isso: mandando o valor direto, as formas
// 1 e 2 responderam "ponteiro invalido", ou seja, trataram o numero como
// endereco e foram le-lo. Entao o handle vai logo depois da struct e a uniao
// aponta para ele, do mesmo jeito que a forma de texto faz.
//
// A forma continua sendo parametro porque o cabecalho do SDK nao esta
// disponivel e os exports sao undecorated: nao da para deduzir a constante do
// binario, so medindo.
func novaInputHandleNativa(forma, handle uint32) uintptr {
	p := nativoAloca(12)
	if p == 0 {
		return 0
	}
	escritas := []struct {
		...
		{p, forma},             // Form
		{p + 4, uint32(p + 8)}, // uniao: ponteiro para o handle
		{p + 8, handle},        // o handle em si
	}
	for _, e := range escritas {
		if err := escreveUint32Nativo(e.onde, e.valor); err != nil {
			nativoLibera(p)
			return 0
		}
	}
...
// comparaComEntrada chama um ponto de entrada de comparacao do SDK com dois
// INPUT_FIR ja montados.
//
// Existe para varrer combinacoes de forma e de funcao procurando uma que o
// gancho de API do servidor RDP nao quebre. O gancho intercepta os simbolos que
// conhece; um irmao menos usado pode passar limpo, e isso e a diferenca entre
// ter conserto e depender do fornecedor.
//
// comPayload distingue as duas assinaturas: NBioAPI_VerifyMatch recebe um
// ponteiro de payload no fim, NBioAPI_CompareTwoFIR nao.
func (n *nbio) comparaComEntrada(proc *syscall.LazyProc, nome string, inA, inB uintptr, comPayload bool) (bool, error) {
	if err := proc.Find(); err != nil {
		return false, fmt.Errorf("%s ausente nesta DLL: %w", nome, err)
	}
	saida := nativoAloca(4)
	defer nativoLibera(saida)
	if inA == 0 || inB == 0 || saida == 0 {
	...
	var r uintptr
	if comPayload {
		r, _, _ = proc.Call(n.h, inA, inB, saida, 0)
	} else {
		r, _, _ = proc.Call(n.h, inA, inB, saida)
	}
	if r != 0 {
		return false, novoErroSDK(uint32(r), nome)
	}
	resultado, err := leUint32Nativo(saida)
	if err != nil {
		return false, err
...
// capturaHandle captura e devolve o handle cru, sem converter para texto e sem
// liberar. Quem chama e responsavel por chamar liberaFIR.
func (n *nbio) capturaHandle(purpose uint16, timeoutMs int32) (uint32, error) {
	if err := n.abre(); err != nil {
		return 0, err
	}
	defer func() {
	...
	r, _, _ := n.capture.Call(n.h, uintptr(purpose), saidaFIR, uintptr(uint32(timeoutMs)), 0, 0)
	if r != 0 {
		return 0, novoErroSDK(uint32(r), "NBioAPI_Capture")
	}
	hFIR, err := leUint32Nativo(saidaFIR)
	if err != nil {
		return 0, err
	...
	if hFIR == 0 {
		return 0, errors.New("NBioAPI_Capture devolveu handle nulo")
	}
	return hFIR, nil
}

func (n *nbio) liberaFIR(handle uint32) {
	if handle != 0 {
		n.freeFIR.Call(n.h, uintptr(handle))
	}
}

// textoDoHandle converte um handle em texto sem liberar o handle.
func (n *nbio) textoDoHandle(handle uint32) (string, error) {
	texto := nativoAloca(tamTextEncode)
	if texto == 0 {
		return "", errors.New("sem memoria nativa")
	}
	defer nativoLibera(texto)
	r, _, _ := n.getText.Call(n.h, uintptr(handle), texto, 0)
	if r != 0 {
		return "", novoErroSDK(uint32(r), "NBioAPI_GetTextFIRFromHandle")
	}
	ponteiro, err := leUint32Nativo(texto + offsetTextEncodePtr)
